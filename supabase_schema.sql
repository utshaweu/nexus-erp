-- ============================================================
-- NexusERP Multi-Tenant Schema (Option A — Tenant-per-Row)
-- ============================================================
...
-- New clients = INSERT one row. Nothing else changes.
...
-- ── 2. TENANT USERS ──────────────────────────────────────────
-- Maps Supabase auth users to a tenant + role.
...
-- ── 4. HELPER FUNCTIONS ──────────────────────────────────────
-- current_tenant_id() — used in every RLS policy (DRY)
...
-- is_super_admin() — bypasses tenant isolation in admin panel
CREATE OR REPLACE FUNCTION is_super_admin()
RETURNS BOOLEAN LANGUAGE sql STABLE AS $$
  SELECT COALESCE(
    (auth.jwt()->'user_metadata'->>'is_super_admin')::BOOLEAN, false
  );
$$;
...
-- ── 8. BUSINESS TABLES (all carry tenant_id) ─────────────────
CREATE TABLE vendors (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  name TEXT NOT NULL, contact_name TEXT, email TEXT, phone TEXT,
  address TEXT, country TEXT, category TEXT,
...
CREATE TABLE purchase_orders (
  ...
  expected_date DATE, reference TEXT, notes TEXT,
  subtotal NUMERIC(15,2) DEFAULT 0, tax_amount NUMERIC(15,2) DEFAULT 0,
...
CREATE TABLE purchase_order_lines (
  ...
  product_name TEXT NOT NULL, quantity NUMERIC(10,2) NOT NULL,
  unit_price NUMERIC(15,2) NOT NULL, tax_rate NUMERIC(5,2) DEFAULT 0,
...
CREATE TABLE rfqs (
  ...
  vendor_id UUID REFERENCES vendors(id),
  status TEXT DEFAULT 'draft' CHECK (status IN ('draft','sent','received','expired','converted')),
  deadline DATE, notes TEXT, quoted_amount NUMERIC(15,2),
  created_by UUID REFERENCES auth.users(id),
...
CREATE TABLE customers (
  ...
  tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  name TEXT NOT NULL, contact_name TEXT, email TEXT, phone TEXT,
  address TEXT, country TEXT, industry TEXT,
...
CREATE TABLE sales_orders (
  ...
  status TEXT DEFAULT 'draft' CHECK (status IN ('draft','confirmed','invoiced','done','cancelled')),
  order_date DATE NOT NULL DEFAULT CURRENT_DATE,
  delivery_date DATE, salesperson_id UUID REFERENCES auth.users(id),
  notes TEXT, subtotal NUMERIC(15,2) DEFAULT 0,
  tax_amount NUMERIC(15,2) DEFAULT 0, total_amount NUMERIC(15,2) DEFAULT 0,
  created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now(),
  UNIQUE (tenant_id, order_number)
);
...
CREATE TABLE sales_order_lines (
  ...
  product_name TEXT NOT NULL, quantity NUMERIC(10,2) NOT NULL,
  unit_price NUMERIC(15,2) NOT NULL, discount_pct NUMERIC(5,2) DEFAULT 0,
  tax_rate NUMERIC(5,2) DEFAULT 0, total_price NUMERIC(15,2)
);
...
CREATE TABLE warehouses (
  ...
  capacity_pct NUMERIC(5,2) DEFAULT 0, status TEXT DEFAULT 'active',
  created_at TIMESTAMPTZ DEFAULT now(),
...
CREATE TABLE products (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  sku TEXT NOT NULL, name TEXT NOT NULL, description TEXT, category TEXT,
  unit_of_measure TEXT DEFAULT 'unit',
  cost_price NUMERIC(15,2) DEFAULT 0, sale_price NUMERIC(15,2) DEFAULT 0,
...
CREATE TABLE stock_moves (
  ...
  move_type TEXT NOT NULL CHECK (move_type IN ('incoming','outgoing','internal','adjustment')),
  quantity NUMERIC(10,2) NOT NULL,
  source_warehouse_id UUID REFERENCES warehouses(id),
  dest_warehouse_id UUID REFERENCES warehouses(id),
  reference TEXT, move_date DATE DEFAULT CURRENT_DATE,
  created_by UUID REFERENCES auth.users(id),
  created_at TIMESTAMPTZ DEFAULT now(),
...
CREATE TABLE invoices (
  ...
  subtotal NUMERIC(15,2) DEFAULT 0, tax_amount NUMERIC(15,2) DEFAULT 0,
  total_amount NUMERIC(15,2) DEFAULT 0, paid_amount NUMERIC(15,2) DEFAULT 0,
...
CREATE TABLE bills (
  ...
  bill_date DATE DEFAULT CURRENT_DATE, due_date DATE,
  total_amount NUMERIC(15,2) DEFAULT 0, paid_amount NUMERIC(15,2) DEFAULT 0,
  created_at TIMESTAMPTZ DEFAULT now(),
...
CREATE TABLE departments (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  name TEXT NOT NULL, code TEXT, manager_id UUID,
...
CREATE TABLE employees (
  ...
  position TEXT, manager_id UUID REFERENCES employees(id),
  employment_type TEXT DEFAULT 'full_time'
    CHECK (employment_type IN ('full_time','part_time','contract','intern')),
  status TEXT DEFAULT 'active' CHECK (status IN ('active','inactive','terminated')),
  hire_date DATE, salary NUMERIC(15,2), avatar_url TEXT,
  created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now(),
...
CREATE TABLE leave_requests (
  ...
  days_count NUMERIC(4,1), reason TEXT,
...
CREATE TABLE asset_categories (
  ...
  useful_life_years INT, salvage_value_pct NUMERIC(5,2) DEFAULT 0,
  created_at TIMESTAMPTZ DEFAULT now(),
  UNIQUE (tenant_id, code)
);
...
CREATE TABLE assets (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  asset_number TEXT NOT NULL, name TEXT NOT NULL, description TEXT,
  category_id UUID REFERENCES asset_categories(id),
  purchase_date DATE, purchase_cost NUMERIC(15,2) NOT NULL,
  book_value NUMERIC(15,2), salvage_value NUMERIC(15,2) DEFAULT 0,
  useful_life_years INT, depreciation_method TEXT DEFAULT 'straight_line',
  location TEXT, assigned_to UUID REFERENCES employees(id),
  status TEXT DEFAULT 'active'
    CHECK (status IN ('active','maintenance','disposed','fully_depreciated')),
  serial_number TEXT, warranty_expiry DATE,
  vendor_id UUID REFERENCES vendors(id),
  created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now(),
...
CREATE TABLE asset_depreciation_logs (
  ...
  opening_value NUMERIC(15,2), depreciation_amount NUMERIC(15,2),
  closing_value NUMERIC(15,2), created_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE asset_maintenance_logs (
  ...
  description TEXT, cost NUMERIC(15,2) DEFAULT 0, performed_by TEXT,
  maintenance_date DATE DEFAULT CURRENT_DATE, next_maintenance_date DATE,
  created_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE approval_workflows (
  ...
  trigger_condition TEXT, is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE approval_workflow_steps (
  ...
  step_order INT NOT NULL, step_name TEXT NOT NULL,
...
CREATE TABLE approval_requests (
  ...
  title TEXT NOT NULL, description TEXT, module TEXT NOT NULL,
  record_id UUID, record_type TEXT, amount NUMERIC(15,2),
...
CREATE TABLE approval_actions (
  ...
  comment TEXT, acted_at TIMESTAMPTZ DEFAULT now()
);

-- ── 9. BULK RLS — all business tables ────────────────────────
DO $$
DECLARE tbl TEXT;
BEGIN
...
CREATE INDEX ON approval_requests(tenant_id, status, requested_by);

-- ── 11. ORDER-NUMBER GENERATORS ──────────────────────────────
...
-- ── 12. DEMO SEED DATA ───────────────────────────────────────
-- After running this schema, insert your demo tenants:
--
-- INSERT INTO tenants (name, slug, plan) VALUES
--   ('Prince Bazar', 'prince-bazar', 'growth'),
--   ('Agora',        'agora',        'starter');
--
-- Then in Supabase Dashboard > Auth > Users, create:
--   admin@prince-bazar.com  (user_metadata: { "role": "admin" })
--   admin@agora.com         (user_metadata: { "role": "admin" })
--   superadmin@nexuserp.com (user_metadata: { "is_super_admin": true })
--
-- Then INSERT into tenant_users linking those auth UUIDs to their tenants.
